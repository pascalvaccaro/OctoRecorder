...
class Param:
    ...
    def from_internal(self, idx: int, msg: InternalMessage):
        ...
        yield SysexCmd("patch", [idx, self.address, *values])

    def to_internal(self, idx: int, data: "list[int]"):
        yield MacroMessage(
            self.name, idx, self.macro, self.to_vel(data[abs(min(0, self.offset))])
        )
...
class Switch(Pot):
    def from_internal(self, idx: int, msg: MacroMessage):
        ...
        if value > self.min_value:
            yield SysexCmd("patch", [idx, self.address, 1, value])
        else:
            yield SysexCmd("patch", [idx, self.address, 0])
...
class LFO(Pot):
    ...
    def from_internal(self, idx: int, msg: MacroMessage):
        value = self.from_vel(msg.data[2])
        if value > self.min_value:
            yield SysexCmd("patch", [idx, self.address, 1, self.shape, value])
        else:
            yield SysexCmd("patch", [idx, self.address, 0])
...
class Bipolar(Pot):
    ...
    def to_vel(self, ftype: int, value: int):
        if ftype == 0:
